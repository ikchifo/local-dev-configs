...
detect_repo() {
    ...
    # Check if this is a Go project (single repo with go.mod at root)
...
get_build_command() {
    ...
    # Special case for Go root project
    if [[ "$repo" == "go-root" ]]; then
...
# Function to get Go test command for repo
get_go_test_command() {
    local repo="$1"
    local project_root="$CLAUDE_PROJECT_DIR"

    # Only for Go root projects
    if [[ "$repo" == "go-root" ]]; then
...
# Function to get Go vet command for repo
get_go_vet_command() {
    local repo="$1"
    local project_root="$CLAUDE_PROJECT_DIR"
...
go_test_cmd=$(get_go_test_command "$repo")
go_vet_cmd=$(get_go_vet_command "$repo")
...
if [[ -n "$go_test_cmd" ]]; then
    echo "$repo:go-test:$go_test_cmd" >> "$cache_dir/commands.txt.tmp"
fi

if [[ -n "$go_vet_cmd" ]]; then
    echo "$repo:go-vet:$go_vet_cmd" >> "$cache_dir/commands.txt.tmp"
fi
